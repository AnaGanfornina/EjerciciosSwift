...
func compress(sequence: [Int],
              initialValue: Int,
              combinationOperation: IntCombiner,completion:() -> ()) -> Int? {
...
//let numbers = [1, 2, 3, 4]
//let result = compress(sequence: numbers,initialValue: 0,combinationOperation: { $0 + $1 }) {
     //print("¡Operación completada!")
 //}
...
func squareSumAndPrintWithCompress(sequence: [Int]) -> Int?{
    return compress(sequence: sequence, initialValue: 0, combinationOperation: {$0 + $1 * $1 }) {
        print(("squareSumAndPrintWithCompress operation"))
    }
}
let numbers1 = [1, 2, 3]
let result1 = squareSumAndPrintWithCompress(sequence: numbers1)
...
func sumMultiplesOfThreeAndPrintWithCompress(sequence: [Int]) -> Int? {
    compress(sequence: sequence, initialValue: 0, combinationOperation: {$1 % 3 == 0 ? $0 + $1 : $0}) {
        sleep(4)
