...
func sumAll(sequence: [Int]) -> Int {
    sequence.reduce(0) { partialResult, number in
...
func multiplyAll(sequence: [Int]) -> Int {
    sequence.reduce(0) { partialResult, num in
        partialResult * num
    }
}
...
/*
compress(sequence: numbers2, initialValue: 0) { accum, number in
    accum + number
}
*/
compress(sequence: numbers2, initialValue: 0) { $0 + $1 }
...
func sumWithCompress(sequence: [Int]) -> Int {
    return compress(sequence: sequence, initialValue: 0, combinationOperation: { $0 + $1 })
}
//let numbers = [1, 2, 3, 4, 5]
//let result = sumWithCompress(sequence: numbers)
...
func multiplyWithCompress(sequence: [Int]) -> Int {
    return compress(sequence: sequence, initialValue: 1, combinationOperation: { $0 * $1 })
}
...
func sumEvenWithCompress(sequence: [Int]) ->Int {
    var evenSequence: [Int] = []
    for number in sequence{
        if number % 2 == 0 {
            evenSequence.append(number)
        }
    }
    return compress(sequence: evenSequence, initialValue: 0, combinationOperation: { $0 + $1 })
}
//let numbers = [1, 2, 3, 4, 5, 6]
//let result = sumEvenWithCompress(sequence: numbers)
...
func sumEvensSubtractOddsWithCompress(sequence: [Int]) -> Int {
    let sumEvens = sumEvenWithCompress(sequence: sequence)
    var oddSequence: [Int] = []
    for number in sequence{
        if number % 2 != 0 {
    ...
    let sumOdds = compress(sequence: oddSequence, initialValue: 0, combinationOperation: { $0 + $1 })
...
func compress1(sequence: [Int], initialValue: Int, combinationOperation: IntCombiner) -> Int {
    var accum = initialValue
    for number in sequence{
        accum = combinationOperation(accum,number)
    }
...
func maxElementWithCompress(sequence: [Int]) -> Int {
    ...
    compress(sequence: sequence, initialValue: Int.min) { acum, number in
        if number >= acum {
            return number
        }else{
            return acum
...
func minElementWithCompress(sequence: [Int]) -> Int {
    ...
    compress(sequence: sequence, initialValue: Int.max) { acum, number in
        if number <= acum {
            return number
        }else{
            return acum
...
/// print(resultEmpty) // Output: 0
/// ```
func compress2(sequence: [Int], initialValue: Int, combinationOperation: IntCombiner) -> Int {
    var accum = initialValue
    for number in sequence{
        accum = combinationOperation(accum,number)
    }
...
func allEvensWithCompress(sequence:[Int]) -> Bool{
    ...
    let lengthSequence = sequence.count
    
    let count = compress(sequence: sequence, initialValue: 0, combinationOperation: { $1 % 2 == 0 ? $0 + 1 : $0})
    return count == lengthSequence
}
...
func allMultiplesOfThreeWithCompress(sequence: [Int]) -> Bool {
    ...
    let count = compress(sequence: sequence, initialValue: 0, combinationOperation: { $1 % 3 == 0 ? $0 + 1 : $0})
    return count == lengthSequence
}
...
/// ```
func sumAllEvensAndOddsWithCompress(sequence:[Int]) -> (Int,Int) {
    let evens = compress(sequence: sequence, initialValue: 0, combinationOperation: { $1 % 2 == 0 ? $0 + $1 : $0})
    let odds = compress(sequence: sequence, initialValue: 0, combinationOperation: { $1 % 2 != 0 ? $0 + $1 : $0})
    return (evens,odds)
}
...
func compress3(sequence: [Int], initialValue: Int, combinationOperation: IntCombiner) -> Int {
    var accum = initialValue
    for number in sequence{
        accum = combinationOperation(accum,number)
    }
...
func compressWithoutInitalValue(sequence: [Int], combinationOperation: IntCombiner) -> Int? {
    guard !sequence.isEmpty else{
        return nil
    }
    var accum = sequence.first!//sequence[0]
...
let numbers = [5, 2, 3, 4]
let sum = compressWithoutInitalValue(sequence: numbers) { $0 + $1 }
print(sum) // Output: Optional(10)
